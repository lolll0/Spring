    ...
    public void contextInitialized(ServletContextEvent ctxEvent)  { 
      ...
      try {
         ...
         Map<String, Object> applicationContext = 
               ApplicationContext.getApplicationContext(); // application contexㅅt
         ...
         // 의존주입
         for(int i = 0; i< beans.getLength(); i++) { //한번 더 beans를 찾고
            Node bean = beans.item(i);
            if(bean.getNodeType() == Node.ELEMENT_NODE) {
               Element eleBean = (Element)bean;
               
               NodeList properties = bean.getChildNodes(); // Nodelist에 bean의 자식노드들을 넣고 반복문
